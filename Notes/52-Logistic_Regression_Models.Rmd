...
```{r, out.width = "400px", fig.align = 'center', message = FALSE, warning = FALSE, fig.alt = "A scatterplot between Age (x-axis) and Heart Disease (y-axis) is shown with a simple linear regression model overlayed. All of the Heart disease values are either 0 or 1. As such, the line doesn't really go through the 'middle' of the points indicating that linear regression isn't the best model to use here."}
...
```{r, out.width = "400px", fig.align = 'center', message = FALSE, warning = FALSE,  fig.alt = "A scatterplot between Age (x-axis) and Heart Disease (y-axis) is shown with a simple linear regression model overlayed. The heart disease values are jittered so we can see them more easily. In this setup, we can see how the line sort of goes throught he points, but clearly this isn't the ideal model in this situation."}
ggplot(heart_data, aes(x = Age, y = HeartDisease, color = RestingBP)) +
         geom_jitter() +
  geom_smooth(method = "lm")
```
...
```{r, out.width = '400px', fig.align = 'center',fig.alt = "A scatterplot between Age (x-axis) and Heart Disease (y-axis) is shown with a simple linear regression model overlayed. The heart disease values are jittered so we can see them more easily. The proportion of those with heart disease is also plotted. The regression line roughly models these means, indicating there is some usefulness in modeling the means with a line, but this model still isn't ideal."}
...
```{r, echo = FALSE, fig.align='center', out.width = '400px', message = FALSE, warning = FALSE, fig.alt = "A plot of the proportion of those with heart disease for different age groups is plotted along with the logistic regression fit overlayed. The logistic model is non-linear and roughly goes through these sample proportions."}
...
```{r, echo = FALSE, fig.align='center', out.width = '400px', message = FALSE, warning = FALSE,  fig.alt = "A plot of the proportion of those with heart disease for different age groups is plotted along with the logistic regression fit overlayed. The logistic model is non-linear and roughly goes through these sample proportions. The plot is zoomed further out and the non-linearity is more present. Predictions never go below 0 and never above 1."}
...
```{r, echo = FALSE, fig.align='center', out.width = '400px', message = FALSE, warning = FALSE, fig.alt = "A plot of the proportion of those with heart disease for different age groups, grouped by sex, is plotted along with the logistic regression fits overlayed. "}
Age_x <- seq(from = min(heart_data$Age), to = max(heart_data$Age), length = 20)
...
```{r, echo = FALSE, fig.align='center', out.width = '400px', message = FALSE, warning = FALSE, fig.alt = "A plot of the proportion of those with heart disease for different age groups, grouped by sex, is plotted along with the logistic regression fits overlayed. This plot is more zoomed out and it is clear that the difference in prediction is not a simple numeric shift but separate fitted curves."}
...
```{r, echo = FALSE, out.width = '200px', fig.align = 'center', fig.alt = "An example logit model with logit equal to -1 +3*x-x^2 is shown. This model starts near 0, rises, and then drops back down towards 0 indicating that our logistic model does not have to start low and end high with more complicated predictors included."}
x <- seq(-1, 5, 0.01)
b0 <- -1
b1 <- 3
b2 <- -1
