...
1.  describe the idea of coercion and compare and contrast the use of implicit and explicit coercion
...
1.  describe the process of a for loop, while loop, and repeat statement in R
...
2.     program using the "apply family" of functions in R
...
3.     recall common "vectorized" functions in R such as colMeans, rowMeans, ifelse, etc.

4.     write their own vectorized version of a function in R (CO 1, 4)

5.     describe what an anonymous function is and how it differs from other functions

### Writing Functions

Note: Not all of these are covered in this week's function writing videos - we'll revisit the topic later!

1.  display code from previously written functions in R

2.  describe the parts of an R function

3.  describe the difference between a prefix and an infix function

    a.  understand the order of evaluation for user created and built-in infix functions

4.  write an R function that has default arguments, allows for additional arguments, and returns a well-structured named object
...
5.  describe the way that inputs can be specified to a function
...
7.  utilize the switch function and compare and contrast its use against if/then/else logic

8.  define environments including temporary environments
