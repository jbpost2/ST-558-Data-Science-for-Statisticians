...
```{r, fig.align='center', out.width='400px', fig.alt = "A basic tree model is shown that first splits on speed (less than 17.5 or larger) and then has further splits down each of those lines. The final value at the end of each tree is the mean for that group of observations."}
...
```{r, message = FALSE, warning = FALSE, fig.align='center', out.width='400px', fig.alt = "A scatterplot between speed (x-axis) and distance (y-axis is shown). The simple linear regression model is shown overlayed along with a step function representing the tree model fit is shown."}
library(tidyverse)
...
```{r, fig.align='center', out.width='400px', fig.alt = "A scatterplot between speed (x-axis) and distance (y-axis is shown)."}
ggplot(cars, aes(x = speed, y = dist)) + 
  geom_point()
```
...
```{r, echo = FALSE, fig.align='center', out.width='400px', fig.alt = "A scatterplot between speed (x-axis) and distance (y-axis is shown). An example split at x = 7 shows the SSE for this split is 27665.5."}
...
g <- ggplot(data = cars, aes(x = speed, y = dist)) + 
  ...
  geom_segment(data = SSE1$info, aes(x = split, xend = split, y = 0, yend = 85), 
               lwd = 0.5, color = "Blue", lty = "dashed") +
...
```{r, echo = FALSE, fig.align='center', out.width='400px', fig.alt = "A scatterplot between speed (x-axis) and distance (y-axis is shown). An example split at x = 7 shows the SSE for this split is 27665.5, a split at x = 12 shows the SSE is 19380.8, a split at x = 15 shows the SSE is 19559, and a split at x = 20 gives SSE of 19595."}
#plot and put on   
g <- g + 
  ...
  geom_segment(data = SSE2$info, aes(x = split, xend = split, y = 0, yend = 85), 
               lwd = 0.5, color = "Red", lty = "dashed") +
...
g <- g +   
  ...
  geom_segment(data = SSE3$info, aes(x = split, xend = split, y = 0, yend = 85), 
               lwd = 0.5, color = "Orange", lty = "dashed") +
...
g <- g +   
  ...
  geom_segment(data = SSE4$info, aes(x = split, xend = split, y = 0, yend = 85), 
               lwd = 0.5, color = "Brown", lty = "dashed") +
...
```{r, echo = FALSE, fig.align='center', out.width='400px', fig.alt = "A scatterplot between speed (x-axis) and distance (y-axis is shown). An best split at x = 17.5 shows the SSE for this split is 17322.5."}
g2 <- ggplot(data = cars, aes(x = speed, y = dist)) + 
  ...
  geom_segment(data = SSE$info, aes(x = split, xend = split, y = 0, yend = 85), 
               lwd = 0.5, color = "Blue", lty = "dashed") +
...
```{r, echo = FALSE, fig.align='center', out.width='400px', fig.alt = "A scatterplot between speed (x-axis) and distance (y-axis is shown). The data is first split at x = 7. After that split, splits on the lower side are shown with their corresponding SSE values. This is the process of fitting a regression tree."}
cars_r1 <- cars |>
  filter(speed < 17.5)
vals <- c(7, 12, 15)
...
g3 <- ggplot(data = cars, aes(x = speed, y = dist)) + 
  ...
  geom_segment(data = SSE1$info, aes(x = split, xend = split, y = 0, yend = 55), 
               lwd = 0.5, color = "Red", lty = "dashed") +
...
g3 <- g3 +
  ...
  geom_segment(data = SSE2$info, aes(x = split, xend = split, y = 0, yend = 55), 
               lwd = 0.5, color = "Orange", lty = "dashed") +
...
g3 <- g3 +
  ...
  geom_segment(data = SSE3$info, aes(x = split, xend = split, y = 0, yend = 55), 
               lwd = 0.5, color = "Brown", lty = "dashed") +
...
Next, let's define our model. The <a href = "https://parsnip.tidymodels.org//reference/details_linear_reg_lm.html" target = "_blank">info page</a> here can be used to determine the right function to call and the possible engines to use for the fit.

In this case, `decision_tree()` with `rpart` as the engine will do the trick. If we click on the <a href = "https://parsnip.tidymodels.org//reference/details_decision_tree_rpart.html" target = "_blank">link for this model</a> we can see that there are three tuning parameters we can consider:
...
```{r, fig.alt = "A plot of the metric RMSE vs cost complexity and R-squared vs cost complexity are shown. The values of the metrics for different values of the tuning parameter tree depth are plotted."}
tree_fits |>
  collect_metrics() |>
...
```{r, fig.alt = "A tree diagram is shown that represents the model fit. This shows all the splits at different variables and the corresponding prediction within the nodes."}
tree_final_model |>
  extract_fit_engine() |>
