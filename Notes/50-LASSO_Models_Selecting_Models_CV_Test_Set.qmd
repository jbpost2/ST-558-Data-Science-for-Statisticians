...
- [Least Angle Subset and Selection Operator](https://www.jstor.org/stable/2346178) or LASSO
  ...
  + For an $\alpha$ of 0 we get the usual least squares estimates. As $\alpha$ gets larger we 'shrink' the estimates toward 0.
  
```{r, echo = FALSE, fig.align = 'center', out.width = "450px", fig.alt = "A graph of the standardized regression coefficients as a function of alpha is shown. On the left (small alpha), the coefficients are all non-zero. As the value of alpha increases, the values of the coefficients shrink toward zero with some being set to zero earlier than others. However, for a large enough alpha, all the coefficients are set to zero."}
knitr::include_graphics("img/lasso_path.png")
```

- We have a different model fit for each value of $\alpha$. When choosing the tuning parameter ($\alpha$), we are really considering a **family of models** for a given set of predictors!
  
  - Consider the MLR model with different intercepts based on `owners` that includes both `log_km_driven` and `year` as predictors. The plot here gives the coefficient profiles (estimates) as a function of $\alpha$ (although and $\alpha$ of 0 here corresponds to being on the right hand side of the plot).
  
```{r, warning = FALSE, message = FALSE, echo = FALSE, fig.alt = "A plot of the regression coefficients as a function of -log(lambda) is shown. For small values of -log(lambda) the coefficients are all 0 and they grow as the value increases."}
library(glmnet)
...
Great, now we need to specify a LASSO model. Recall <a href = "https://www.tidymodels.org/find/parsnip/" target = "_blank">this page</a> allows us to find a type of model to fit along with how to specify it and the engine used to fit the model.
...
```{r, fig.alt = "A plot of the penalty parameter (x-axis) and RMSE (y-axis) is shown. The value of the metric starts a little higher, drops down and then increases. The minimum value of the curve occurs near a penalty of 0.017."}
LASSO_grid |>
  collect_metrics() |>
  filter(.metric == "rmse") |>
